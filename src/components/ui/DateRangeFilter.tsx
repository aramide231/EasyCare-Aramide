...
import type { DateRangeValue } from "@/lib/dateTime";
...
const toSelection = (value: DateRangeValue): DateRangeSelection[] => [
  { startDate: value.startDate, endDate: value.endDate, key: "selection" },
];
...
const DateRangeFilter = ({ value, onChange, align = "left" }: Props) => {
  ...
  useEffect(() => {
    if (!isOpen) {
      setTempRange(toSelection(value));
    }
  }, [value, isOpen]);
  ...
  return (
    <div ref={containerRef} className="relative w-fit">
      ...
      {isOpen && (
        <div
          className={cn(
            "absolute top-12 z-50 flex rounded-2xl border border-gray-100 bg-white p-6 shadow-xl",
